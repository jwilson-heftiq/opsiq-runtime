...
import { fetchOrderLineWorklist } from "@/lib/api/orderFulfillment"

interface OrderLineWorklistClientProps {
  tenantId: string
...
export function OrderLineWorklistClient({
  tenantId,
  initialData,
  initialState,
  initialSubjectId: initialSubjectIdProp,
}: OrderLineWorklistClientProps) {
  ...
  const applyFilters = (states: Set<DecisionState>, subjectId: string) => {
    startTransition(() => {
      fetchOrderLineWorklist(tenantId, {
            ...
            {data.items.length === 0 ? (
              ...
              data.items.map((item) => (
                <TableRow key={`${item.subject_id}-${item.computed_at}`}>
                  <TableCell>
                    <Link
                      href={`/${tenantId}/order-fulfillment/order-lines/${item.subject_id}`}

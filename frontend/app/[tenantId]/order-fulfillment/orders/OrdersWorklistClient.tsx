...
import { fetchOrdersWorklist } from "@/lib/api/orderFulfillment"

interface OrdersWorklistClientProps {
  tenantId: string
...
const ALL_STATES: DecisionState[] = ["AT_RISK", "NOT_AT_RISK", "UNKNOWN"]

export function OrdersWorklistClient({
  tenantId,
  initialData,
  initialState,
  initialSubjectId: initialSubjectIdProp,
}: OrdersWorklistClientProps) {
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Line Counts</TableHead>
          ...
          <TableBody>
            {data.items.length === 0 ? (
              ...
              data.items.map((item) => {
                ...
                const total = metrics?.order_line_count_total ?? 0
                const atRisk = metrics?.order_line_count_at_risk ?? 0
                const unknown = metrics?.order_line_count_unknown ?? 0
                const notAtRisk = metrics?.order_line_count_not_at_risk ?? 0
                
                return (
                  <TableRow key={`${item.subject_id}-${item.computed_at}`}>
                    <TableCell>
                      <Link
                        href={`/${tenantId}/order-fulfillment/orders/${item.subject_id}`}

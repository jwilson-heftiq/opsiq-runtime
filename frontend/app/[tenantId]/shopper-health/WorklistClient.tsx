...
import { fetchShopperHealthWorklistClient } from "@/lib/api/decisions"
...
const ALL_STATES: DecisionState[] = ["URGENT", "WATCHLIST", "HEALTHY", "UNKNOWN"]
...
export function WorklistClient({
  ...
  const applyFilters = (states: Set<DecisionState>, subjectId: string) => {
    startTransition(() => {
      fetchShopperHealthWorklistClient(tenantId, {
            ...
            {data.items.length === 0 ? (
              ...
              data.items.map((item) => (
                <TableRow key={`${item.subject_id}-${item.computed_at}`}>
                  <TableCell>
                    <Link
                      href={`/${tenantId}/shopper-health/${item.subject_id}`}
                  ...
                  <TableCell>
                    <DecisionStateBadge state={item.decision_state} />
                  </TableCell>
                  <TableCell>
